...
AGunBullet::AGunBullet()
{
	...
	BoxComponent = CreateDefaultSubobject<UBoxComponent>("BulletComponent");
	BoxComponent->SetCollisionEnabled(ECollisionEnabled::QueryAndPhysics);
	BoxComponent->SetCollisionProfileName("Fireball");
	BoxComponent->SetNotifyRigidBodyCollision(true);
	BoxComponent->SetSimulatePhysics(false);
	BoxComponent->OnComponentHit.AddDynamic(this, &AGunBullet::OnHit);
	...
	CheckOverlap->OnComponentBeginOverlap.AddDynamic(this, &AGunBullet::OnCheckOverlap);
	CheckOverlap->SetupAttachment(RootComponent);

	FlipbookComponent = CreateDefaultSubobject<UPaperFlipbookComponent>("BulletFlipbook");
...
void AGunBullet::Spawn()
{
	...
	{
		Velocity.X = -GameConstants::BulletSpeed;
	...
	{
		Velocity.Z = GameConstants::BulletSpeed;
		FlipbookComponent->SetRelativeRotation(FRotator(90.0f, 0.0f, 0.0f));
		SetActorLocation(FVector(marioloc.X, 0.0f, marioloc.Z + 24.0f));
	}
}
...
void AGunBullet::OnHit(UPrimitiveComponent* HitComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, FVector NormalImpulse, const FHitResult& Hit)
{
	if (OtherActor->ActorHasTag("Enemy") || OtherActor->ActorHasTag("World") || OtherActor->ActorHasTag("ItemBox"))
	{
		LifeTime = 0.05f;
	}
}

void AGunBullet::OnCheckOverlap(UPrimitiveComponent* OverlapComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	if (OtherActor->ActorHasTag("Enemy") || OtherActor->ActorHasTag("World") || OtherActor->ActorHasTag("ItemBox"))
	{
		LifeTime = 0.05f;
